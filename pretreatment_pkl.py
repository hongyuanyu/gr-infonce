...
parser.add_argument('--input_path', default='/home2/ywc/workspace/GaitDatasetB-silh', type=str,
                    help='Root path of raw dataset.')
parser.add_argument('--output_path', default='/home2/ywc/workspace/output_pkl_64', type=str,
...
def cut_pickle(seq_info, pid):
    ...
    # frame_list.sort()
    frame_list = pickle.load(open(os.path.join(seq_path, 'sil_path_list.pkl'), 'rb'))
    count_frame = 0
    view = seq_info[-1]
    ...
    for frame_path in frame_list:
        _frame_name = os.path.basename(frame_path)
        ...
        if img is not None:
            ...
            # save_path = os.path.join(out_dir, _frame_name[:-3]+'pkl')
            # tmp_fp = open(save_path, 'wb')
            # pickle.dump(img, tmp_fp)
            # tmp_fp.close()
            all_imgs.append(img)
            all_img_names.append(_frame_name)
            count_frame += 1
    all_imgs = np.asarray(all_imgs)
    all_imgs_pkl = os.path.join(out_dir, '{}.pkl'.format(view))
    pickle.dump(all_imgs, open(all_imgs_pkl, 'wb'))
    # all_img_names_pkl = os.path.join(out_dir, 'bboxes_name.pkl')
    # pickle.dump(all_img_names, open(all_img_names_pkl, 'wb'))
    # cmd = "cp {}/bboxes_loc_dict.pkl {}".format(seq_path, out_dir)
    # os.system(cmd)
