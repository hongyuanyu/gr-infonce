...
m.init_model('{}'.format(config['ckp_prefix']))
print('Transforming...')
print('#######################################')
print('Feat_idx={}'.format(config['feat_idx']))
print('#######################################')
time = datetime.now()
eval_feature_pkl = '{}_{}set_{}eval_feature.pkl'.format(config['dataset'], config['test_set'], os.path.split(config['ckp_prefix'])[-1])
if config['resume'] and os.path.exists(eval_feature_pkl):
    ...
    pickle.dump(data, open(eval_feature_pkl, 'wb'))
